...
# calculate statistical significance over time 
def max_val(dataset):
    """ Calculate max value of a time series dataset.  
    
        Args:
        dataset (xarray): time series dataset over which to calculate the max
        """
...
# print max values and max dates
def max_vals(data, var=None):
    
    """Function to calculate and print maximum value in a time series. 
    ...
    # print max value
    print(f'Maximum value of {var} signal on each spatial scale is:')
    
    # calculate and print max value at each spatial scale
    for region, ts in data.items():
        ...
        # find date of max
        max_date = min_date(ts, max_val).dt.strftime("%b %Y")
        
        if var == 'temperature':
            print(f'{region}:', max_val.values.round(decimals=2), f'{deg}C', f'in {max_date.data}')
        
        elif var == 'precipitation':
            print(f'{region}:', max_val.values.round(decimals=3), 'mm/day', f'in {max_date.data}')
        
        else:
            print(f'{region}:', max_val.values.round(decimals=4), f'in {max_date.data}')
...
# print max values and max dates
def max_vals_sea(data, var=None):
    ...
    for region, ts in data.items():
        ...
        # find date of max
        max_date = min_date_sea(ts, max_val)
        
        if var == 'temperature':
            print(f'{region}:', max_val.values.round(decimals=2), f'{deg}C', f'{max_date.data} months post-eruption')
        
        elif var == 'precipitation':
            print(f'{region}:', max_val.values.round(decimals=3), 'mm/day', f'{max_date.data} months post-eruption')
        
        else:
            print(f'{region}:', max_val.values.round(decimals=4), f'in {max_date.data} months post-eruption')
        
    return



# print seasonal values for summary table
def vals_seasonal(data, var=None, dates=None):
    
    """Function to calculate and print seasonal value in a time series. 
    
        Args:
        data (xarray): [] array of time series data at each spatial scale
        dates [start_date, end_date] (optional): array with start and end date of desired print values
        var (string) (optional): string of varaible name
    """
    ...
    # print max value
    print(f'Value of {var} signal on each spatial scale is:')
    
    # calculate and print max value at each spatial scale
    for region, ts in data.items():
        
        # group by season
        ts_seasonal = seasonal_group(ts).sel(seasonyear=slice(1883,1886))
        
        if var == 'temperature':
            print(f'{region}:')
            
            # print DJF values
            for vals in ts_seasonal.sel(season='DJF'):
                print(f'{vals.season.data} {vals.seasonyear.data}:', vals.values.round(decimals=3), f'{deg}C')
            
            #print JJA values
            for vals in ts_seasonal.sel(season='JJA'):
                print(f'{vals.season.data} {vals.seasonyear.data}:', vals.values.round(decimals=3), f'{deg}C')
            ...
            # print DJF values
            for vals in ts_seasonal.sel(season='DJF'):
                print(f'{vals.season.data} {vals.seasonyear.data}:', vals.values.round(decimals=3), f'mm/day')
            ...
            # print DJF values
            for vals in ts_seasonal.sel(season='DJF'):
                print(f'{vals.season.data} {vals.seasonyear.data}:', vals.values.round(decimals=3))
...
# print monthly values for summary table
def vals_monthly(data, var=None, dates=None):
    ...
    if dates is None:
        dates = ['1884-02', '1884-08', '1885-02', '1885-08', '1886-02', '1886-08']
    ...
    for region, ts in data.items():
        ...
        if var == 'temperature':
            ...
            for d in dates:
                print(d, ts.sel(time=d).values.round(decimals=3), f'{deg}C')
            ...
            # print date and value
            for d in dates:
                print(d, ts.sel(time=d).values.round(decimals=3), f'mm/day')
            ...
            # print date and value
            for d in dates:
                print(d, ts.sel(time=d).values.round(decimals=3))
